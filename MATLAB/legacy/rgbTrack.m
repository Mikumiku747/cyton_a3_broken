function rgbTrack(redThresh,greenThresh,blueThresh,blob)
...
% greenThresh = 0.25; % Threshold for green detection
...
while(vid.FramesAcquired<=200)
    ...
    redDifference = imsubtract(redChannel, (greenChannel + blueChannel)/2);
    greenDifference = imsubtract(greenChannel, (redChannel + blueChannel)/2);
    blueDifference = imsubtract(blueChannel, (redChannel + greenChannel)/2);   
    ...
    snap_red = bwareaopen(snap_red, blob);
    snap_blue = bwareaopen(snap_blue, blob);
    snap_green = bwareaopen(snap_green, blob);   
